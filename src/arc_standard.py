...
class arcStandard(object):
	...
	def nextTransition(self):
		...
		if len(self.stack) == 0: 
			...
			if s0.pos in self.template["LEFTARC"] and b0.pos in self.template["LEFTARC"][s0.pos][0][0]: # NOTE exceptions
				if len(self.queue) is 1: return self.LEFTARC, self.template["LEFTARC"][s0.pos][-1]
				...
				else: return self.RIGHTARC , self.template["RIGHTARC"][b0.pos][-1]
			else: return self.SHIFT, None

	def hasDependents (self, b0):
		for c in self.queue[1:]:
			cI = self.sequence[c]
			if cI.pos in self.template["LEFTARC"] and b0.pos in self.template["LEFTARC"][cI.pos][0][-1]: return True
		return False
